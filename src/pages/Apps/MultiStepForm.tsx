...
const MultiStepForm: React.FC = () => {
  const { currentStep, finished } = useMultiStepFormStore((s) => s)
  ...
  return (
    <main className="w-full max-w-screen min-h-dvh bg-[#f0f5ff]">
      <div className="w-full bg-[url('/multi-step-form/bg-sidebar-mobile.svg')] h-50 bg-center bg-cover bg-no-repeat py-10 relative">
        ...
        <section className="w-[calc(100%-48px)] max-w-xl bg-white rounded-lg shadow-lg absolute top-7/12 left-1/2 transform -translate-x-1/2 p-6">
          {!finished && (
